...
function townhub_addons_registration_callback() {
    ...
    if ( ! is_wp_error( $user_id ) ) {
        ...
        if( $registered_as_author && $free_lplan = townhub_addons_get_option('free_lplan') ){
            Esb_Class_Form_Handler::insert_free_subscription( $free_lplan, $user_id );
...
function townhub_addons_send_resetpwd_email($user_obj,$key){
    // Redefining user_login ensures we return the right case in the email.
    ...
    $message = apply_filters( 'esb_reset_password_email', $message, $user_obj );
