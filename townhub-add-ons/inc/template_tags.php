...
function townhub_addons_get_user_role_name($role = ''){
    ...
    global $wp_roles;
    return translate_user_role( $wp_roles->roles[ $role ]['name'] , 'default');
}
// not used yet
function townhub_addons_get_author_roles(){
    ...
    foreach ( $editable_roles as $role => $details ) {
        ...
        if( 'administrator' !== esc_attr( $role ) ) $roles[esc_attr( $role )] = translate_user_role( $details['name'] , 'townhub-add-ons');
    }
...
function townhub_addons_limit_upload_size( $file ) {
    ...
    if ( ! current_user_can( 'manage_options' ) ) {
        ...
        if ( $current_size > $file_size_limit ) {
            $file['error'] = sprintf( __( 'ERROR: File size limit is %d MB.', 'townhub-add-ons' ), $file_size_limit );
        }
