...
class Esb_Class_Form_Handler{
    ...
    public static function add_to_booking(){
        ...
        if(is_numeric($listing_id) && (int)$listing_id > 0){
            ...
            $booking_title = __( '%1$s booking request by %2$s', 'townhub-add-ons' ); 
            $booking_datas = array();
            ...
            if (!is_wp_error($booking_id)) {
                ...
                $bkdatas['price_total'] = $listing_price;
                // $cmb_prefix = '_cth_';
    ...
    public static function insert_free_subscription($plan_id = 0, $user_id = 0 ){
        $cart_data = array(
            'product_id'                => $plan_id,
            'subtotal'                  => 0,
            'subtotal_vat'              => 0,
    ...
    public static function add_to_cart_action(){
        ...
        if($adding_post->post_type == 'lplan'){
            ...
            $bkfmenus = (isset($_POST['bkfmenus']) && !empty($_POST['bkfmenus'])) ? $_POST['bkfmenus'] : array();
