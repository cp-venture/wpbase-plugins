...
class Esb_Class_Install{
    ...
    public static function update(){
        ...
        if(!wp_doing_ajax()){
            ...
            if( version_compare(get_option( 'townhub-addons-version' ), '1.2.0', '<') ){
                
                self::leventdate_start();
                update_option( 'townhub-addons-version', '1.2.0' );
            }
            if( version_compare(get_option( 'townhub-addons-version' ), '1.2.3', '<') ){
                self::lmenu_id();
                update_option( 'townhub-addons-version', '1.2.3' );
            }
        }
    }
            ...
            foreach ( $listings as $lid ) {
                ...
                if( !empty($old_menus) && is_array($old_menus) ){
                    ...
                    foreach ($old_menus as $menu) {
                        $menu['_id'] = 'lmenu_' .substr(str_shuffle(str_repeat($x='0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', ceil(11/strlen($x)) )),1,11);
                        $new_menus[] = $menu;
