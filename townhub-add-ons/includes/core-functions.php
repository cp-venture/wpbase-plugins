...
function townhub_addons_listing_author_no_admin_access() {
    if(!wp_doing_ajax()){
        ...
        //     // $error = new WP_Error( 'error', __( "You don't have permission to access this page.", 'townhub-add-ons' ) );
        ...
        if( in_array( townhub_addons_get_user_role(), townhub_addons_get_option('admin_bar_hide_roles') ) ){
    ...
    foreach ($terms as $term) {
        ...
        if ( ! empty( $term_obj ) && ! is_wp_error( $term_obj ) ){
            ...
            if( $max_level != '' && $level < $max_level ){
                $ccats = townhub_addons_filter_child_cats($term_obj->term_id, $level+1 , $max_level, $hide_empty);
...
function townhub_addons_get_listing_marker($post_id = 0){
    $post_thumbnail_id = '';
    if( townhub_addons_get_option('use_osm_map') != 'yes' && townhub_addons_get_option('use_dfmarker') != 'yes' ){
        $post_thumbnail_id = get_post_thumbnail_id( $post_id );
        if( !empty($post_thumbnail_id) ){
            return $post_thumbnail_id;
        }
