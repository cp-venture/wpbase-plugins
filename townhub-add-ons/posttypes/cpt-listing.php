...
class Esb_Class_Listing_CPT extends Esb_Class_CPT {     
    ...
    public function submit_listing() {
        ...
        if(isset($_POST['locations']) && $_POST['locations']){
            ...
            if(!empty($plocs)){
                ...
                if( !$country_term ) {
                    $country_term = wp_insert_term( townhub_addons_get_google_contry_codes($country), 'listing_location', array( 'parent' => 0 , 'slug' => $country ) );
                }
        ...
        if(isset($_POST['select_locations'])&& !empty( $_POST['select_locations'] ) ){
            if(!is_array($_POST['select_locations'])){
                $listing_data['tax_input']['listing_location'] = array( esc_html($_POST['select_locations']) );
            }else{
            ...
            if($child_pt != 'lrooms' && $child_pt != 'none'){
                ...
                // product_image_gallery
                 if( isset($room_post['images']) && $room_post['images'] ){
                    ...
                    update_post_meta( $room_id, 'product_image_gallery', $room_post['images'] );
                 }
    ...
    public static function parse_wkhours($post_ID = 0){
        ...
        $open_text = _x( 'Now Opening', 'Open text', 'townhub-add-ons' );
            ...
            foreach ($working_days as $day => $dayLbl) {
                ...
                if( isset($dayVals['static']) ){
                    if( $dayVals['static'] == 'closeAllDay' ){
                        ...
                        //             if( $ophrAfter > $clhrAfter && $curHr < $clhrAfter ){
                        //                 $return['statusText'] = __( 'Now Opening',  'townhub-add-ons' );
                        //                 $return['status'] = 'opening';
                        //             }
                        //         }
                        //     }
