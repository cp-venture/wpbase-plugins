...
add_action( 'wp_footer', function(){
    ...
    if( is_singular( 'listing' ) ){
        ...
        if( is_user_logged_in() ) townhub_addons_get_template_part('template-parts/listing-report-modal');
    }
} );
...
if(!function_exists('townhub_addons_login_logout_sc')){
    function townhub_addons_login_logout_sc($atts, $content = ''){
        ...
        if ( 0 == $current_user->ID ) {
            // Not logged in.
            ?>
            <div class="show-reg-form avatar-img logreg-modal-open"><?php _e('<i class="fal fa-user"></i>Sign In','townhub-add-ons');?></div>
        <?php
        } else {
            // Logged in.
            //$dashboard_page_id = esb_addons_get_wpml_option('dashboard_page');
