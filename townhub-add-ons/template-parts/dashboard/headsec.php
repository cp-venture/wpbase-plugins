...
if( $user_role == 'listing_author' && ( $plan_id = Esb_Class_Membership::current_plan( get_current_user_id() ) ) != '' ){
    $plan_string = sprintf(__('<span>Tariff Plan: </span> <strong>%1$s</strong>', 'townhub-add-ons'), get_the_title($plan_id));
    $plan_desc = sprintf(__('<p>You are on <a>%1$s</a>. Use link bellow to view details or upgrade.</p><a class="tfp-det-btn color2-bg" href="%2$s">Details</a>', 'townhub-add-ons'), get_the_title($plan_id), get_permalink( townhub_addons_get_option('packages_page') ) );
}else{
    $plan_string = sprintf(__('<span>Your are: </span> <strong>%1$s</strong>', 'townhub-add-ons'), townhub_addons_get_user_role_name());
    $plan_desc = sprintf(__('<p>You are <a>%1$s</a>. Order an membership plan to submit listings.</p><a class="tfp-det-btn color2-bg" href="%2$s">Membership Plans</a>', 'townhub-add-ons'), townhub_addons_get_user_role_name(), get_permalink( townhub_addons_get_option('packages_page') ) );
}
